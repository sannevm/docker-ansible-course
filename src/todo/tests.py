...
class TestCreateTodoItem(APITestCase):
    ...
    def test_item_has_correct_title(self):
        self.assertEqual(TodoItem.objects.get().title, 'Do the dishes')

class TestUpdateTodoItem(APITestCase):
    def setUp(self):
        repsonse = createItem(self.client)
        self.assertEqual(TodoItem.objects.get().completed, False)
        url = repsonse['Location']
        data = {'title': 'Do the dishes', 'completed': True}
